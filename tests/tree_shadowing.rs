...
#[test]
fn tree_shadowing_prefix_with_prefix_extractor() -> lsm_tree::Result<()> {
    const ITEM_COUNT: usize = 10_000;

    let folder = tempfile::tempdir()?.into_path();

    let tree = Config::new(folder)
        .prefix_extractor(Arc::new(TestPrefixExtractor::new(4)))
        .open()?;
...
#[test]
fn tree_shadowing_prefix_blob_with_prefix_extractor() -> lsm_tree::Result<()> {
    const ITEM_COUNT: usize = 10_000;

    let folder = tempfile::tempdir()?.into_path();

    let mut cfg = Config::new(folder);
    cfg.prefix_extractor = Some(Arc::new(TestPrefixExtractor::new(4)));
    let tree = cfg.open()?;

...
fn tree_shadowing_upsert() -> lsm_tree::Result<()> {
    ...
    assert_eq!(tree.get(key, None)?, Some(value.into()));
...
fn tree_shadowing_prefix() -> lsm_tree::Result<()> {
    ...
    assert_eq!(
        tree.prefix("pre".as_bytes(), None, None).count(),
        ITEM_COUNT * 2
    );
    assert_eq!(
        tree.prefix("prefix".as_bytes(), None, None).count(),
        ITEM_COUNT
    );
    assert!(tree
        .iter(None, None)
